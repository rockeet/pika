...
int zset_string2d(const char *s, size_t slen, double *dval) {
  if ((slen == 4) && (s[0] == '-') && (s[1] == 'i') && (s[2] == 'n') && (s[3] == 'f')) {
    *dval = blackwidow::ZSET_SCORE_MIN;
    return 4;
  }
  if ((slen == 4) && (s[0] == '+') && (s[1] == 'i') && (s[2] == 'n') && (s[3] == 'f')) {
    *dval = blackwidow::ZSET_SCORE_MAX;
    return 4;
  }
  ...
  return slash::string2d(s, slen, dval);
}

int zset_d2string(char *buf, size_t len, double value) {
  assert(len > 4);
  if (value == blackwidow::ZSET_SCORE_MIN) {
    buf[0] = '-'; buf[1] = 'i'; buf[2] = 'n'; buf[3] = 'f'; buf[4] = 0;
    return 4;
  } else if (value == blackwidow::ZSET_SCORE_MAX) {
    buf[0] = 'i'; buf[1] = 'n'; buf[2] = 'f'; buf[3] = 0;
    return 3;
  } else {
    return slash::d2string(buf, len, value);
  }
}
...
void ZAddCmd::DoInitial() {
  ...
  for (; index < argc; index += 2) {
    if (!zset_string2d(argv_[index].data(), argv_[index].size(), &score)) {
...
void ZAddCmd::Do(const std::shared_ptr<Partition>& partition) {
  int32_t count = 0;
...
void ZCardCmd::Do(const std::shared_ptr<Partition>& partition) {
  int32_t card = 0;
...
void ZScanCmd::Do(const std::shared_ptr<Partition>& partition) {
  ...
  if (s.ok() || s.IsNotFound()) {
    ...
    for (const auto& score_member : score_members) {
      ...
      len = zset_d2string(buf, sizeof(buf), score_member.score);
      res_.AppendStringLen(len);
      res_.AppendContent(buf);
    }
  } else {
...
void ZIncrbyCmd::DoInitial() {
  ...
  key_ = argv_[1];
  if (!zset_string2d(argv_[2].data(), argv_[2].size(), &by_)) {
    res_.SetRes(CmdRes::kInvalidFloat);
    return;
  }
  member_ = argv_[3];
...
void ZIncrbyCmd::Do(const std::shared_ptr<Partition>& partition) {
  double score = 0;
  rocksdb::Status s = partition->db()->ZIncrby(key_, member_, by_, &score);
  if (s.ok()) {
    char buf[32];
    int64_t len = zset_d2string(buf, sizeof(buf), score);
    res_.AppendStringLen(len);
    res_.AppendContent(buf);
  } else {
    res_.SetRes(CmdRes::kErrOther, s.ToString());
...
void ZRangeCmd::Do(const std::shared_ptr<Partition>& partition) {
  std::vector<blackwidow::ScoreMember> score_members;
  ...
  if (s.ok() || s.IsNotFound()) {
    if (is_ws_) {
      ...
      for (const auto& sm : score_members) {
        ...
        len = zset_d2string(buf, sizeof(buf), sm.score);
        res_.AppendStringLen(len);
        res_.AppendContent(buf);
      }
    } else {
...
void ZRevrangeCmd::Do(const std::shared_ptr<Partition>& partition) {
  std::vector<blackwidow::ScoreMember> score_members;
...
int32_t DoScoreStrRange(std::string begin_score, std::string end_score, bool *left_close, bool *right_close, double *min_score, double *max_score) {
  ...
  } else if (!zset_string2d(begin_score.data(), begin_score.size(), min_score)) {
    return -1;
  } 
  ...
  } else if (!zset_string2d(end_score.data(), end_score.size(), max_score)) {
    return -1;
  }
  return 0;
}
...
void ZRangebyscoreCmd::Do(const std::shared_ptr<Partition>& partition) {
  ...
  if (with_scores_) {
    ...
    for (; index < end; index++) {
      ...
      len = zset_d2string(buf, sizeof(buf), score_members[index].score);
      res_.AppendStringLen(len);
      res_.AppendContent(buf);
    }
  } else {
...
void ZRevrangebyscoreCmd::Do(const std::shared_ptr<Partition>& partition) {
  if (min_score_ == blackwidow::ZSET_SCORE_MAX || max_score_ == blackwidow::ZSET_SCORE_MIN) {
    res_.AppendContent("*0");
    return;
  }
...
void ZCountCmd::Do(const std::shared_ptr<Partition>& partition) {
  if (min_score_ == blackwidow::ZSET_SCORE_MAX || max_score_ == blackwidow::ZSET_SCORE_MIN) {
    res_.AppendContent("*0");
    return;
  }
...
void ZRemCmd::Do(const std::shared_ptr<Partition>& partition) {
  int32_t count = 0;
  ...
  while (index < argc) {
    if (!strcasecmp(argv_[index].data(), "weights")) {
      ...
      for (; index < base + num_keys_; index++) {
        if (!zset_string2d(argv_[index].data(), argv_[index].size(), &weight)) {
...
void ZUnionstoreCmd::Do(const std::shared_ptr<Partition>& partition) {
  int32_t count = 0;
...
void ZInterstoreCmd::Do(const std::shared_ptr<Partition>& partition) {
  int32_t count = 0;
...
void ZRankCmd::Do(const std::shared_ptr<Partition>& partition) {
  int32_t rank = 0;
...
void ZRevrankCmd::Do(const std::shared_ptr<Partition>& partition) {
  int32_t revrank = 0;
...
void ZScoreCmd::Do(const std::shared_ptr<Partition>& partition) {
  double score = 0;
...
void ZRangebylexCmd::Do(const std::shared_ptr<Partition>& partition) {
...
void ZRevrangebylexCmd::Do(const std::shared_ptr<Partition>& partition) {
  if (min_member_ == "+" || max_member_ == "-") {
    res_.AppendContent("*0");
    return;
  }
...
void ZLexcountCmd::Do(const std::shared_ptr<Partition>& partition) {
  if (min_member_ == "+" || max_member_ == "-") {
    res_.AppendContent(":0");
    return;
  }
...
void ZRemrangebyrankCmd::Do(const std::shared_ptr<Partition>& partition) {
  int32_t count = 0;
...
void ZRemrangebyscoreCmd::Do(const std::shared_ptr<Partition>& partition) {
  if (min_score_ == blackwidow::ZSET_SCORE_MAX || max_score_ == blackwidow::ZSET_SCORE_MIN) {
    res_.AppendContent(":0");
    return;
  }
...
void ZRemrangebylexCmd::Do(const std::shared_ptr<Partition>& partition) {
  if (min_member_ == "+" || max_member_ == "-") {
    res_.AppendContent("*0");
    return;
  }
...
void ZPopmaxCmd::Do(const std::shared_ptr<Partition>& partition) {
  std::vector<blackwidow::ScoreMember> score_members;
...
void ZPopminCmd::Do(const std::shared_ptr<Partition>& partition) {
  std::vector<blackwidow::ScoreMember> score_members;
